...
export function getSlideshowHighlights(limitPerYear: number = 3): GalleryImage[] {
  const galleryData = getGalleryData();
  const highlights: GalleryImage[] = [];

  for (const year in galleryData) {
    const events = galleryData[year];
    const yearImages: GalleryImage[] = [];
    for (const eventName in events) {
      yearImages.push(...events[eventName]);
    }

    // Shuffle and pick a few images
    const shuffled = yearImages.sort(() => 0.5 - Math.random());
    highlights.push(...shuffled.slice(0, limitPerYear));
  }
