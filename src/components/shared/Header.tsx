...
const Header: React.FC<HeaderProps> = ({ searchData }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#2f3033]/95 backdrop-blur-md shadow-lg' : 'bg-[#2f3033]/80 backdrop-blur-sm'
      }`}>
            ...
            {navigation.map((item) => (
              <div key={item.path} className="relative group">
                <Link
                  href={item.path}
                  onClick={handleNavClick}
                  className={`px-3 py-2 text-sm font-medium transition-all duration-200 relative group ${currentPath === item.path
                      ? 'text-[#ff8c42] font-bold'
                      : 'text-[#f4f1de] hover:text-[#ffd700]'
                    }`}
                >
                  {item.name}
                  <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-[#ff8c42] transform transition-transform duration-200 ${currentPath === item.path ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                    }`} />
                </Link>
                {item.sublinks && (
                ...
                item.sublinks ? (
                  <Collapsible key={item.path}>
                    <CollapsibleTrigger className="w-full">
                      <div className={`flex justify-between items-center w-full px-3 py-2 text-base font-medium text-left transition-colors duration-200 ${currentPath.startsWith(item.path)
                      ...
                      <div className="pl-4">
                        {item.sublinks.map((sublink) => (
                          <Link
                            ...
                            href={sublink.path}
                            onClick={handleNavClick}
                            className={`block px-3 py-2 text-base font-medium w-full text-left transition-colors duration-200 ${currentPath === sublink.path
                                ? 'text-[#ff8c42] bg-[#454850]/50 font-bold'
                                : 'text-[#f4f1de] hover:text-[#ffd700] hover:bg-[#454850]/30'
                              }`}
                          >
                  ...
                  <Link
                    ...
                    href={item.path}
                    onClick={handleNavClick}
                    className={`block px-3 py-2 text-base font-medium w-full text-left transition-colors duration-200 ${currentPath === item.path
                        ? 'text-[#ff8c42] bg-[#454850]/50 font-bold'
                        : 'text-[#f4f1de] hover:text-[#ffd700] hover:bg-[#454850]/30'
                      }`}
                  >
